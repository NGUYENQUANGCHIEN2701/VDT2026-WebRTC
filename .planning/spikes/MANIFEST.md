...
- **Redis cross-instance routing tăng RTT median ~2× so với same-instance** (303ms → 599ms ở 100 cặp/instance), nhưng con số này CÓ NHIỄU (thứ tự chạy cố định, backend-2 chưa warm-up, cả 2 điều kiện đã ở vùng suy giảm platform-thread) — xem spike 003 để biết cách khử nhiễu nếu cần con số chính xác hơn.
- **Actuator `/actuator/health` tổng có thể báo `DOWN`/Docker báo `unhealthy` vì Mail health indicator** (tài khoản Gmail thật dùng cho OTP bị rate-limit sau nhiều lần restart backend) — không liên quan gì đến DB/Redis/WS. Luôn kiểm tra `readiness`/`liveness` riêng hoặc test chức năng thật (curl login) trước khi kết luận backend "hỏng".
...
| 003 | k6-ws-redis-crossinstance-overhead | standard | Given 2 backend instance + nginx LB, when so sánh cặp client cùng-instance vs khác-instance, then đo delta latency do Redis pub/sub relay | VALIDATED | websocket, redis, cross-instance |
