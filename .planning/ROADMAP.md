...
- [x] 09-02-PLAN.md — Full-stack compose + observability infra: frontend Dockerfile folded into nginx (D-02), Prometheus + Grafana services with provisioning-as-code, VDT WebRTC Overview dashboard
- [x] 09-03-PLAN.md — GitHub Actions CI: backend (mvn verify), frontend (lint+vitest+build), docker-build (no push) as 3 parallel jobs on push/PR to main
